 ...
 * All right reserved.
 */
package com.zw.jmsepc.silkie.epc;
...
public abstract class BaseParam {
	...
	// 系统浮点数，统一采用Double处理，避免JDK float to double的自动转型的bug
	public void setValue(String name, List<Float> value) {
		_setValue(name, flist2Dlist(value));
	}
	// 系统浮点数，统一采用Double处理，避免JDK float to double的自动转型的bug
	public void setValue(String name, Float value) {
		_setValue(name, Double.valueOf(value.toString()));
	}
...
////////////////////////////////////////////////////////////////////////////////	
	...
	private List<Double> flist2Dlist(List<Float> value) {
		List<Double> dlist = null;
		if (value != null) {
			dlist = new ArrayList<Double>(value.size());
			for (int i = 0; i < value.size(); i++) {
				dlist.add(Double.valueOf(value.get(i)));
			}
		}
		return dlist;
	}
	
	/*private List<Float> dlist2Flist(List<Double> value) {
		List<Float> flist = null;
		if (value != null) {
			flist = new ArrayList<Float>(value.size());
			for (int i = 0; i < value.size(); i++) {
				flist.add(DoubleUtil.double2Float(value.get(i)));
			}
		}
		return flist;
	}*/
