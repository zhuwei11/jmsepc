 ...
 * All right reserved.
 */
package com.zw.jmsepc.silkie.epc.impl;
...
import com.zw.jmsepc.silkie.epc.MillsCounter;
...
public class ThreadPoolEpc extends BaseEpc {
	private static final Log log = LogFactory.getLog(ThreadPoolEpc.class);
	...
	ThreadPoolEpc(int pool) {
		this(pool, DEFAULT_MAX_SIZE);
	}
	// 实际使用中发现，线程池的个数不会增长到max，很奇怪
	// 建议初始化时，pool=max
	ThreadPoolEpc(int pool, int max) {
